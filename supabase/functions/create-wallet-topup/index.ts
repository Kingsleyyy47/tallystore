...
async function recordRevenueEvent(
  ...
  const eventType = sanitizeRevenueEventType('create-wallet-topup', input.eventType)
  if (!eventType) return

  const { error } = await supabaseAdmin.from('revenue_events').upsert({
    event_id: await sanitizeRevenueEventId('create-wallet-topup', input.eventId),
  ...
  if (error) {
    console.error(`Failed to record wallet top-up revenue event ${eventType}:`, error.message)
  }
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (profile.is_staff || profile.is_admin) {
      throw new Error('Wallet top-ups are only available to customer accounts.')
    }
    ...
    await recordRevenueEvent(supabaseAdmin, {
      ...
      eventId: `wallet_topup:PAYMENT_STARTED:${transactionReference}`,
      userId: user.id,
      surface: 'wallet_topup',
      revenueContext,
      metadata: {
